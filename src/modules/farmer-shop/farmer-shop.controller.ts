...
export class FarmerShopController {
  ...
  @Post('create')
  async create(
    @Body() createFarmerShopDto: CreateFarmerShopDto,
    ...
    const citizenshipFrontImageUrl =
      files.citizenshipFrontImage && files.citizenshipFrontImage.length > 0
        ? await this.cloudinaryService.uploadFarmerCitizenshipFrontImage(
            files.citizenshipFrontImage[0],
          )
        : '';

    const citizenshipBackImageUrl =
      files.citizenshipBackImage && files.citizenshipBackImage.length > 0
        ? await this.cloudinaryService.uploadFarmerCitizenshipBackImage(
            files.citizenshipBackImage[0],
          )
        : '';

    const farmerShop = await this.farmerShopService.create(
      {
        ...createFarmerShopDto,
  ...
  @UseGuards(RolesGuard)
  @Get('details')
  async getFarmerShop(@User() user: authPayload) {
  ...
  async updateFarmerShopDetails(
    @Body() farmerShop: UpdateFarmerShopDto,
    @User() user: authPayload,
    @UploadedFiles()
    files: {
      shopImage?: Express.Multer.File[];
    ...
    const updatedFarmerShop =
      await this.farmerShopService.updateFarmerShopDetails(
        {
          ...farmerShop,
